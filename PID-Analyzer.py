...
class Trace:
    ...
    def __init__(self, data):
        ...
        self.throt_hist, self.throt_scale = np.histogram(self.throttle, np.linspace(0, 100, 101), density=True)
    ...
    def hist2d(self, x, y, weights, bins):   #bins[nx,ny]
        ...
        int_bins = [int(x) for x in bins]
        hist2d = np.histogram2d(throts.flatten(), freqs.flatten(),
                                range=[[0, 100], [y[0], y[-1]]],
                                bins=int_bins, weights=weights.flatten(), normed=False)[0].transpose()
